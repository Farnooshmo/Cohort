...
class UsernameSpec extends AnyWordSpec with Matchers {
  ...
  //Do this at top level because my class does not take  parameters.
  //Because it is at top level, I van access this anywhere between the yellow {}- class body.
  ...
  "checkUsername" should {
    ...
    "return false" when {
// this is a padding test, but I have written it to prove my method can fail (can get to my else case)
      ...
      "the username is 8 characters long but contains uppercase" in {
        val input = username.checkUsername("Mercator")
        val expectedResult = false // class then method
        input shouldBe expectedResult
      }
      //Edge case
